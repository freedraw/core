...
exports.get = function(name, args) {
  ...
  if (!c) return {
    enabled: false,
    title: autoTitle(name),
    config: {name: name},
  }
  return {
    title: typeof c.title === 'function' ? c.title.apply(c.context, args) : c.title || autoTitle(name),
...
function autoTitle(name) {
  return name[0].toUpperCase() + name.slice(1).replace(/[A-Z]+/g, ' $&')
}
...
Object.defineProperty(exports, 'all', {get: function() {
  var results = []
  exports.each(results.push, results)
  return results
}})
