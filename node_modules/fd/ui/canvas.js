...
function Canvas(editor) {
  ...
  editor.on('documentChange', this.onDocumentChange)
  ...
  commands.add({
    zoom0: {
      title: 'Actual Size',
      fn: this.zoomTo.bind(this, 1, null),
    },
    zoomCenter: {
      title: 'Center Canvas',
      fn: this.zoomCenter,
    ...
    zoomIn: {
      title: 'Zoom In',
      fn: this.zoomBy.bind(this, ZOOM_FACTOR - 1, null),
    },
    zoomOut: {
      title: 'Zoom Out',
      fn: this.zoomBy.bind(this, 1 / ZOOM_FACTOR - 1, null),
    },
  }, this)
  ...
  keyboard.on('modifiersChange', this.onModifiersChange, this)
}

Canvas.prototype.onDocumentChange = function() {
  if (this.document) {
    this.document.unlisten('invalidate', this.onInvalidate, this)
  }
  this.document = this.editor.document
  this.center = this.document.boundingBox.center
  this.document.on('invalidate', this.onInvalidate, this)
}
...
Canvas.prototype.onMouseDown = function(e) {
  ...
  var object = now - old < DOUBLE_CLICK_TIME ? this.hoveredInnerObject : this.hoveredObject
  if (this.editor.selection = object) {
    cursor.push(platform.isWebKit ? '-webkit-grabbing' : '-moz-grabbing')
    this.drag = {
      kind: 'translate',
      rect: this.editor.selection.boundingBox,
...
Canvas.prototype.onModifiersChange = function() {
  this.updateMouse()
}

Canvas.prototype.onSelectionChange = function() {
  this.updateHover()
  this.updateSelectionBox()
}

Canvas.prototype.onInvalidate = function() {
  this.updateHover()
  this.redraw()
}
...
Canvas.prototype.constrainZoom = function() {
  if (this.scale < ZOOM_MIN) this.zoomTo(ZOOM_MIN, mouse.position)
  else if (this.scale > ZOOM_MAX) this.zoomTo(ZOOM_MAX, mouse.position)
}
...
Canvas.prototype.updateMouse = function() {
  var drag = this.drag
  if (!drag) {
    this.updateHover()
    return
  ...
  switch (drag.kind) {
    case 'translate':
      ...
      var v = mouse.position.transform(mat)
      object.boundingBox = drag.rect.translate(v.sub(u))
      this.editor.emit('selectionBoundsChange')
      return
    case 'resize':
      var preserve = keyboard.shift
      var center = keyboard.alt
...
Canvas.prototype.updateHover = function(object) {
  var object = this.hoveredObject
  this.highlightObject(object === this.editor.selection ? null : object)
}
Object.defineProperty(Canvas.prototype, 'hoveredObject', {get: function() {
  if (!this.viewport.includes(mouse.position) || this.drag) return null
  var p = mouse.position.transform(this.screenToModel)
  if (keyboard.alt && keyboard.super) {
    return this.editor.selection
  }
  if (keyboard.super) {
    var n = this.document.nodeAt(p)
    return n === this.document ? null : n
  }
  var s = this.editor.selection
  if (s) p = p.transform(s.globalToLocal)
  return s ? s.ancestorSiblingAt(p) || s.isSolidAt(p) && s : this.document.childAt(p)
}})
Object.defineProperty(Canvas.prototype, 'hoveredInnerObject', {get: function() {
  var parent = this.editor.selection || this.document
  return parent.childAt(mouse.position.transform(this.screenToModel.concat(parent.globalToLocal))) || parent
}})
...
Canvas.prototype.updateSelectionBox = function() {
  ...
  var smallAngle = angle % (Math.PI / 2)
  if (smallAngle < 0) smallAngle += Math.PI / 2
  points.forEach(function(v, i) {
    ...
    if (!hide) {
      handle.style.transform = Matrix.translateBy(v).rotate(smallAngle).toCSS()
