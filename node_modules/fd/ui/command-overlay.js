...
function CommandOverlay(editor) {
  ...
  this.el = h('.command-overlay', [
    this.input = h('input', {placeholder: 'Type a command…'}),
...
CommandOverlay.prototype.update = function() {
  ...
  if (!q) return
  var list = commands.all.map(function(it) {
...
CommandOverlay.prototype.scoreItem = function(it, q) {
  var i = it.title.toLowerCase().indexOf(q.toLowerCase())
  return i === -1 ? i : i + (it.enabled ? 0 : 1e4)
}

CommandOverlay.prototype.addScoredItem = function(o) {
  var it = o.item
  if (!it.config.name) return // TODO
  var binding = bindingFor[it.config.name]
  var c = commands.get(it.config.name)
  this.list.appendChild(h('li.command' + (c.enabled ? ' enabled' : ''), {
    dataset: {name: it.config.name}
  }, [
    h('.title', [
      c.title,
