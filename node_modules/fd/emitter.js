...
function emitter(object) {
  ...
  object.emitThrottled = function(name, data) {
    var map = this.listeners
    if (!map) return this
    var list = map[name]
    if (!list) return this
    var done = this.throttledListeners
    if (!done) this.throttledListeners = done = Object.create(null)
    if (done[name]) return
    done[name] = true
    list.forEach(function(l) {
      asap(l.fn.bind(l.context, data))
    })
    asap(function() {done[name] = false})
    return this
  }

  object.emitImmediate = function(name, data) {
    var map = this.listeners
    if (!map) return []
    var list = map[name]
    if (!list) return []
    return list.map(function(l) {
      return l.fn.call(l.context, data)
    })
  }
