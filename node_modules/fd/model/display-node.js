...
DisplayNode.prototype.shallowNodeAt = function(p) {
  if (this.transform.determinant === 0) return null
  var q = p.transform(this.transform.inverse)

  for (var i = this.children.length; i--;) {
    var c = this.children[i]
    if (c.isDescendantSolidAt(q)) return c
  }

  return this.isSolidAt(q) ? this : null
}
DisplayNode.prototype.isDescendantSolidAt = function(p) {
  if (this.transform.determinant === 0) return false
  var q = p.transform(this.transform.inverse)
  if (this.isSolidAt(q)) return true

  for (var i = this.children.length; i--;) {
    if (this.children[i].isDescendantSolidAt(q)) return true
  }
  return false
}
